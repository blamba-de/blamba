 ...
 * 
 * Modified by Manawyrm for use in Blamba!
 * - support for a decodePDUIntoDiv(div, pdu) function call, with multiple SMS being decoded into 
 * several HTML-div-tags at the same time (scoping, etc.)
 * - modded for fixed only user data, UTF-8, UDH operation
 * 
 * @constructor
 */
    'use strict';

    function decodePDUIntoDiv(div, pdu)
    {
        var $output = div;
        ...
        $( 'table.data > tbody', $output ).prepend( '<tr><td colspan="2" class="one">Some information hidden - click here to reveal.</td></tr>' );
        $( 'table.data tr.hideable', $output ).hide();
        $( 'table.data td.one', $output ).click( function() {
            $( 'table.data td.one', $output ).parent().remove();
            $( 'table.data tr.hideable', $output ).show();
        } );
    ...
    function constructOutput( pdu ) {
        ...
        if (typeof data === 'object') {
            for (i = 0; i < data.length; ++i) {
                datastr += '<tr><td>' + data[ i ].replace( /\t/, '</td><td style="word-break: break-all;">' ) + '</td></tr>';
            }
        }
        else {
        ...
        UDH: function( octets ) {
            ...
            for (i = 0; i < IEs.length; ++i) {
                if (IEs[ i ].IEI === 5) {
                    ...
                    text = 'WDP (Wireless Datagram Protocol): <br>Destination port is ' + destPort + ', <br>source port is ' + (IEs[ i ].IED[2] * 256 + IEs[ i ].IED[3]);
